...
export default function WelcomePopup() {
  ...
  return (
    <div className="popup welcome-popup" style={{display: isClosed ? 'none' : 'block'}}>
      <button aria-label="Close" className="close-btn" onClick={togglePopup}><img src={close} alt="Close" /></button>
        ...
        <div className="cont">
          <div className="text-content">
            ...
            <div className="row direction-row m-t-5">
              <button className="btn btn-secondary" onClick={closeAndSetIsVisited}>Utforska</button>
              <Link to="/about" className="btn btn-tertiary" onClick={setIsVisited}>Om Äventyrskartan</Link>
            </div>
          </div>
        </div>
      </div>
